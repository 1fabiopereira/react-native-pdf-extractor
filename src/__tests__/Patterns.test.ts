...
describe('Patterns', () => {
    describe('Common', () => {
        describe('Email', () => {
            const cases = Patterns.Common.Email.map((_, index): [string, string, boolean][] => {
                return [
                    ['match', Patterns.Common.Email[index], true],
                    ['not match', Patterns.Common.Email[index], false],
                ]
            }).flat()
            it.each(cases)('Should %s with pattern %s', (_: string, __: string, value: boolean) => {
                const email = value ? 'name.lastname@provider.com' : 'email.provedor.com'
                const patterns = Patterns.Common.Email.map((pattern) => new RegExp(pattern))
    
                const match = patterns
                    .map((pattern) => pattern.test(email))
    ...
    describe('Brazil', () => {
        describe('Bank Slip', () => {
            ...
            const cases = Patterns.Brazil.BankSlip.map((_, index): [string, string, boolean, number][] => {
                return [
            ...
            it.each(cases)('Should %s with pattern %s', (_: string, __: string, value: boolean, index: number) => {
                const data = value ? formats[index] : wrongFormats[index]
                const patterns = Patterns.Brazil.BankSlip.map((pattern) => new RegExp(pattern))
    
                const match = patterns
                    .map((pattern) => pattern.test(data))
                    .filter((result: boolean) => result)
                    .length > 0
