...
describe('Test React Native Pdf Extractor', () => {
    ...
    describe('Methods', () => {
        ...
        ])('Should call native modules PdfExtractor %s function %s times with params %s', async (fn, times, params) => {
           const executors = (new Array(times).fill(fn)).map((f) => functions.get(f))

           Promise.all(executors.map(async (exec) => await exec(...params)))
            .then(() => {
